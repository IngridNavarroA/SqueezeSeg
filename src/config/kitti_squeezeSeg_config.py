...
def kitti_squeezeSeg16_config_red():
  """ Parameters to tune after adding one class """
  mc                    = base_model_config('KITTI')
  mc.CLASSES            = ['unknown', 'car', 'person', 'ground']
  mc.NUM_CLASS          = len(mc.CLASSES)
  mc.CLS_2_ID           = dict(zip(mc.CLASSES, range(len(mc.CLASSES))))
  mc.CLS_LOSS_WEIGHT    = np.array([1/15.0, 1.0,  10.0, 1/10.0])
  mc.CLS_COLOR_MAP      = np.array([[ 0.00,  0.00,  0.00],
                                    [ 0.12,  0.56,  0.37],
                                    [ 0.58,  0.72,  0.88], 
                                    [ 0.25,  0.51,  0.76]])
  ...
  # Not sure if I need to change this 
  mc.BI_FILTER_COEF     = 0.1
  mc.BILATERAL_THETA_A  = np.array([.9,   .9,   .6,  .9])
  mc.BILATERAL_THETA_R  = np.array([.015, .015, .01, .015])
  mc.ANG_THETA_A        = np.array([.9,   .9,   .6, .9])
  mc.ANG_FILTER_COEF    = 0.02
